...
// NOTE: switching a provisioned device from the kiosk build to the normal
// one requires giving up device ownership FIRST (the escape gesture, which
// calls clearDeviceOwnerApp). The normal flavour ships neither
// KioskAdminReceiver nor a working controller, so a device owner recorded
// against a component that no longer exists can no longer be cleared by any
// means short of a factory reset.
